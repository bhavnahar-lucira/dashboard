...
// Sync history for one smart sort. Read-only: the backend writes a
// smart_sort_runs doc per sync (status running -> completed/failed).
...
export function SyncsModal({ rule, runs, loading, onClose }) {
  if (!rule) return null;
      ...
      <div className='bg-white rounded-[2.5rem] w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl'>
        <div className='px-8 py-5 border-b border-zinc-100 bg-zinc-50/50 flex items-center justify-between gap-4'>
          <div className='min-w-0'>
            <h2 className='text-lg font-bold text-zinc-900 truncate'>Sync history — {rule.collectionTitle || rule.collectionHandle}</h2>
            <p className='text-[11px] text-zinc-400 mt-0.5'>
              &ldquo;0 moves&rdquo; is normal — Shopify is only written when a product&apos;s position actually changes.
            </p>
          </div>
        ...
        <div className='flex-1 overflow-y-auto px-8 py-6 custom-scrollbar'>
          {loading ? (
            ...
            <p className='text-sm text-zinc-400 text-center py-12'>No syncs yet — use Sync now, or wait for the daily schedule.</p>
          ) : (
              ...
              <table className='w-full text-xs'>
                <thead>
                  <tr className='text-left text-[10px] font-black uppercase tracking-widest text-zinc-400 border-b border-zinc-100'>
                    ...
                    <th className='py-2 pr-4'>Sort switched</th>
                ...
                <tbody>
                  {runs.map((run) => (
                    <tr key={run._id} className='border-b border-zinc-50'>
                      ...
                      <td className='py-2.5 pr-4 text-zinc-600'>{run.totalProducts}</td>
                      <td className='py-2.5 pr-4 text-zinc-600'>{run.moves}</td>
                      <td className='py-2.5 pr-4 text-zinc-500'>{run.sortOrderChanged ? 'to manual' : '—'}</td>
